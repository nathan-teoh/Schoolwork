...
public class Hashtable {
    ...
    public Object get(Object key) {
    	int pos = this.hash(key);	//get hash val for key
    	if(this.table[pos]!=null) {	//if array at index pos isnt empty
    		LinkedList<HashtableRecord> hashRec = (LinkedList<HashtableRecord>) this.table[pos];	//create tmp var
    		for(int i = 0; i < hashRec.size(); i++) {	//iterate through linked list in the array index
    			if(hashRec.get(i).key.equals(key)) {	//if they match
    				return hashRec.get(i).value;	//returns match value
    			}
    		} 		
    	}
       return "null"; // runs if array index i is empty/null or does not find a match.  	
    }
